...
from sqlalchemy import JSON, Boolean, DateTime, ForeignKey, Index, String, Table, Column, func, UniqueConstraint
from sqlalchemy.dialects.postgresql import ARRAY, UUID
from sqlalchemy.orm import Mapped, mapped_column

# Portable string-array column: PG ARRAY on postgres, JSON list everywhere else.
...
class FieldMask(Base):
    """Field-level masking rule.

    Semantics: every row says "users WITHOUT codename `unmask_codename` cannot
    see these `fields` on entity_type X". Multiple rows for the same
    entity_type union together; admin always bypasses.
    """

    __tablename__ = "acl_field_masks"
    __table_args__ = (
        UniqueConstraint("unmask_codename", "entity_type", name="uq_field_mask_code_entity"),
        Index("ix_field_masks_entity", "entity_type"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    unmask_codename: Mapped[str] = mapped_column(String(128), nullable=False)
    entity_type: Mapped[str] = mapped_column(String(64), nullable=False)
    fields: Mapped[list[str]] = mapped_column(PortableStringList, nullable=False, default=list)
    description: Mapped[str | None] = mapped_column(String(500))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
